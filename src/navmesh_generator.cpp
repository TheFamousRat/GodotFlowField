...
void DetourNavigationMeshGenerator::build()
{
    ...
    std::cout << "	Joint build : " << std::chrono::duration_cast<std::chrono::milliseconds>( end - start).count() << '\n';
    ...
    std::cout << "	Build tiles : " << std::chrono::duration_cast<std::chrono::milliseconds>( end - start).count() << '\n';
}
...
{	
	...
	{
		//We refresh/build the GridMap in that area
		const int nvp = poly_mesh->nvp;
		const float cs = poly_mesh->cs;
		const float ch = poly_mesh->ch;
		const float* orig = poly_mesh->bmin;
		float minGridCellSize = std::min(gridmap->get_cell_size().x, std::min(gridmap->get_cell_size().y, gridmap->get_cell_size().z));//Disgusting. But works

		//Cleaning the rasterized tile in the GridMap, since we rebuilt the mesh there
		gridmap->clearTile(x, z);

		//We iterate through the polygons
		for (int i = 0; i < poly_mesh->npolys; ++i)
		{
			const unsigned short* p = &poly_mesh->polys[i*nvp*2];

			// Iterate the triangles
			const unsigned short* v_0ref = &poly_mesh->verts[p[0]*3];
			Vector3 v_0 = Vector3(orig[0] + v_0ref[0]*cs, orig[1] + v_0ref[1]*ch, orig[2] + v_0ref[2]*cs);
			const unsigned short* v_nref = &poly_mesh->verts[p[1]*3];
			Vector3 v_n = Vector3(orig[0] + v_nref[0]*cs, orig[1] + v_nref[1]*ch, orig[2] + v_nref[2]*cs);
			for (int j = 0; j < nvp - 2; ++j)
			{ 
				if (p[j+2] == RC_MESH_NULL_IDX) 
					break; // End of vertices.
				
				const unsigned short* v_n1ref = &poly_mesh->verts[p[j+2]*3];
				Vector3 v_n1 = Vector3(orig[0] + v_n1ref[0]*cs, orig[1] + v_n1ref[1]*ch, orig[2] + v_n1ref[2]*cs);

				gridmap->rasterizeTileTriangle(v_0, v_n, v_n1, x, z);
		...
		/**
		 * Once the GridMap has been built for this tile, we clean it
		 * That means removing the tiles with a filled tile above them
		 */
		gridmap->clearSuperposedCellsInTile(x, z);
	}
